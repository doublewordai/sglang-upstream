/**
 * \file topk_decode_floor_dbg_vunroll.cuh
  ...
  // ---------------- P1: the single streaming read + capture ----------------
  {
    ...
    for (uint32_t u = g; u < total; u += G) {
      ...
      if (p.aligned) {
        ...
        const int64_t rounds = (n4 + 4 * kBlock - 1) / (4 * kBlock);
        for (int64_t r = 0; r < rounds; ++r) {
          const int64_t base_g = r * 4 * kBlock + tid;
          float x[4][4];
          int64_t gs[4];
#pragma unroll
          for (int u = 0; u < 4; ++u) {
            gs[u] = base_g + u * kBlock;
            if (gs[u] < n4) load4<T>(rowp + beg + gs[u] * 4, x[u]);
          }
#pragma unroll
          for (int u = 0; u < 4; ++u) {
            if (gs[u] < n4) {
#pragma unroll
              for (int e = 0; e < 4; ++e) process(beg + gs[u] * 4 + e, x[u][e]);
            }
          }
        }
