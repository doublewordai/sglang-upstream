  ...
  // ---------------- P1: the single streaming read + capture ----------------
  {
    ...
    for (uint32_t u = g; u < total; u += G) {
      ...
      if (p.aligned) {
        ...
        // uniform tiles of kBlock float4 groups; the flush guard covers
        // 4*kBlock appends per tile, so slots never reach kStage
        for (int64_t tile = 0; tile * kBlock < n4; ++tile) {
          __syncthreads();
          if (tid == 0 && s_stage_n > static_cast<int>(kStage) - kStageGuard) s_do_flush = 1;
          __syncthreads();
          if (s_do_flush) { flush_stage(); if (tid == 0) s_do_flush = 0; }
          __syncthreads();
          const int64_t grp = tile * kBlock + tid;
          if (grp < n4) {
            float x[4];
            load4<T>(rowp + beg + grp * 4, x);
#pragma unroll
            for (int e = 0; e < 4; ++e) process(beg + grp * 4 + e, x[e]);
          }
        }
        for (int64_t i = beg + (n4 << 2) + tid; i < end; i += kBlock)
          process(i, to_float(rowp[i])); // <= 3 appends: inside the guard headroom
      } else {
        for (int64_t tile = 0; tile * kBlock < (end - beg); ++tile) {
          __syncthreads();
          if (tid == 0 && s_stage_n > static_cast<int>(kStage) - kStageGuard) s_do_flush = 1;
          __syncthreads();
          if (s_do_flush) { flush_stage(); if (tid == 0) s_do_flush = 0; }
          __syncthreads();
          const int64_t i = beg + tile * kBlock + tid;
          if (i < end) process(i, to_float(rowp[i]));
        }
      }
    }
    flush_stage();
      ...
      if (t_sel < 0) {
        ...
        if (!ovfl && suffix_ts >= static_cast<int>(p.topk)) {
          ...
          if (p.aligned) {
            ...
            for (int64_t tile = 0; tile * kBlock < n4all; ++tile) {
              __syncthreads();
              if (tid == 0 && s_stage_n > static_cast<int>(kStage) - kStageGuard) s_do_flush = 1;
              __syncthreads();
              if (s_do_flush) {
              ...
              if (grp < n4all) {
                float x[4];
                load4<T>(rowp + grp * 4, x);
#pragma unroll
                for (int e = 0; e < 4; ++e) fb_process(grp * 4 + e, x[e]);
              }
            }
            for (int64_t i = (n4all << 2) + tid; i < L; i += kBlock)
              fb_process(i, to_float(rowp[i]));
          } else {
            for (int64_t tile = 0; tile * kBlock < L; ++tile) {
              __syncthreads();
              if (tid == 0 && s_stage_n > static_cast<int>(kStage) - kStageGuard) s_do_flush = 1;
              __syncthreads();
              if (s_do_flush) {
              ...
              const int64_t i = tile * kBlock + tid;
              if (i < L) fb_process(i, to_float(rowp[i]));
            }
          }
          __syncthreads();
          fb_store(s_stage_n);
        ...
        if (tid == 0) { s_outB_n = 0; s_rstage_n = 0; s_resid_n = 0; s_do_flush = 0; }
        __syncthreads();
        for (int base = 0; base < n_list; base += kBlock) {
          __syncthreads();
          if (tid == 0 && s_rstage_n > static_cast<int>(kRStage) - kBlock - 8) s_do_flush = 1;
          __syncthreads();
          if (s_do_flush) {
            ...
            if (tid == 0) { s_resid_n += n; s_rstage_n = 0; s_do_flush = 0; }
            __syncthreads();
          }
