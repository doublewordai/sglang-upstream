"""Byte-floor decode-time top-k for DSA indexers (see jit/csrc/dsa/topk_decode_floor_dbg_nopdl.cuh).
...
@cache_once
def _jit_topk_decode_floor_dbg_nopdl_module():
    return load_jit(
        "dsa_topk_decode_floor_dbg_nopdl",
        "false",
        cuda_files=["dsa/topk_decode_floor_dbg_nopdl.cuh"],
        cuda_wrappers=[("topk_decode_floor_dbg_nopdl", "TopKDecodeFloor<false>::run")],
    )
...
def topk_decode_floor_dbg_nopdl(
    scores: torch.Tensor,
    ...
    module = _jit_topk_decode_floor_dbg_nopdl_module()
    module.topk_decode_floor_dbg_nopdl(scores, lengths, out, page_table, ws, cap, stats)
