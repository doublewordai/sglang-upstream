"""Byte-floor decode-time top-k for DSA indexers (see jit/csrc/dsa/topk_decode_floor_dbg_vstream.cuh).
...
@cache_once
def _jit_topk_decode_floor_dbg_vstream_module():
    args = make_cpp_args(is_arch_support_pdl())
    return load_jit(
        "dsa_topk_decode_floor_dbg_vstream_dbg",
        *args,
        cuda_files=["dsa/topk_decode_floor_dbg_vstream.cuh"],
        cuda_wrappers=[("topk_decode_floor_dbg_vstream", f"TopKDecodeFloor<{args}>::run")],
    )
...
def topk_decode_floor_dbg_vstream(
    scores: torch.Tensor,
    ...
    module = _jit_topk_decode_floor_dbg_vstream_module()
    module.topk_decode_floor_dbg_vstream(scores, lengths, out, page_table, ws, cap, stats)
