"""Byte-floor decode-time top-k for DSA indexers (see jit/csrc/dsa/topk_decode_floor_dbg_vnoflush.cuh).
...
@cache_once
def _jit_topk_decode_floor_dbg_vnoflush_module():
    args = make_cpp_args(is_arch_support_pdl())
    return load_jit(
        "dsa_topk_decode_floor_dbg_vnoflush_dbg",
        *args,
        cuda_files=["dsa/topk_decode_floor_dbg_vnoflush.cuh"],
        cuda_wrappers=[("topk_decode_floor_dbg_vnoflush", f"TopKDecodeFloor<{args}>::run")],
    )
...
def topk_decode_floor_dbg_vnoflush(
    scores: torch.Tensor,
    ...
    module = _jit_topk_decode_floor_dbg_vnoflush_module()
    module.topk_decode_floor_dbg_vnoflush(scores, lengths, out, page_table, ws, cap, stats)
