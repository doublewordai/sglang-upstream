...
class LoadSnapshot(msgspec.Struct, omit_defaults=True):
    ...
    # HiSparse host KV pool (decode arm): free/total/pinned host tokens and
    # the cumulative prealloc admission-wait counter (host-pool
    # backpressure). "pinned" = total - free: tokens held by active requests
    # and retained radix rows (locked or evictable) — the occupancy signal a
    # controller can act on BEFORE the pool limit (cf. CONCUR: cache
    # efficiency degrades mid-phase, not only at capacity).
