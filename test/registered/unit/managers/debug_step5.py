...
for key in outs_sync:
    ...
    if not same:
        ...
        break
print("tables first 8 idx (req0):")
print("sync(0,0):", outs_sync[(0, 0)][0, :8].tolist())
print("pre (0,0):", outs_pre[(0, 0)][0, :8].tolist())
print("sync(1,0):", outs_sync[(1, 0)][0, :8].tolist())
print("pre (1,0):", outs_pre[(1, 0)][0, :8].tolist())
print("tokens p0 :", [int(x) for x in tks[0][0, :8].tolist()])
db = self.sync.coordinator.req_device_buffer_token_locs[0][8, :16].tolist()
print("sync dblocs[req8][:16]:", db)
db2 = self.sync.coordinator.req_device_buffer_token_locs[1][8, :16].tolist()
print("sync dblocs L1 [req8][:16]:", db2)
stash = self.prefetch.coordinator._verify_slot_table[0][: rpi.shape[0]].clone()
print("stash[0] now:", stash[0, :8].tolist())
mc = self.prefetch.coordinator._miss_count_v
print("miss counts per pos now:", [mc[p, :2].tolist() for p in range(N_POS)])
